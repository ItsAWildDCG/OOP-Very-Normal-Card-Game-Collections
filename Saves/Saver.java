...
public class Saver {
    public static void save(SaveData data) {
        try (FileWriter savefile = new FileWriter("Saves/game.sav", false)) {
            ...
            for (Card c : data.current.getHand()) savefile.write(c + "\n");
            savefile.write(data.community.size() + "\n");
            for (Card c : data.community.getHand()) savefile.write(c + "\n");
            savefile.write(data.you.getName() + "\n");
            savefile.write(String.format("%.1f %.1f %d %d %d %d\n", data.you.getAtkMult(), data.you.getDefMult(), data.you.getCurrentChips(), data.you.getBaseChips(), data.you.getVampbuff(), data.you.getBlindbuff()));
            savefile.write(data.items.size() + "\n");
            for (Item i : data.items) savefile.write(i.getName() + "\n");
            savefile.write(data.you.size() + "\n");
            for (Card c : data.you.getHand()) savefile.write(c + "\n");
            savefile.write(data.discardpile.size() + "\n");
            for (Card c : data.discardpile) savefile.write(c + "\n");
            for (Card c : data.dck.check()) savefile.write(c + "\n");
